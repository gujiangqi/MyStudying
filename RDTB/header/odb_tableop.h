...
public:
	...
	cmnInt32	TableCreate (const struct STDB_CREAT_REQ* stdb_creat_req_ptr,
	                         ...
	                         INDEX_DEFINE_ARRAY &index_define_array);  //创建、删除函数,包括索引、虚拟域
#endif
	cmnInt32  TableCreate (const struct STDB_CREAT_REQ* stdb_creat_req_ptr,
	                       const struct FIELD_CREAT_REQ* multi_fields_ptr,
	                       const std::vector<MEMBER_DEFINITION>& vec_offset,
...
public:
	...
	int TableUpdate ( vector<int> field_vec, const char* buf_ptr,
	                  const int buf_size); //关键字域按顺序放在最前面
...
public:
	...
	CBaseIndex *GetIndexPtr (int
	                         seq_no); //返回idx_op_ptr_array的第seq_no个元素，如果第seq_no个元素是NULL的话，new一个元素出来。
	CBaseIndex
	*GetPKIndexPtr();//如果pk_index_ptr不等于NULL，就返回一个pk_index_ptr；否则new一个新的返回。
public:
	int DebugDisp (int index_no,
	               int type); // 显示第index_no个索引的信息，譬如index_no为0、1、......，type为信息的详细程度标志。
	int TableDelete();
